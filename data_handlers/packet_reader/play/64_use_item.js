...
function read(data, length, socket, state) {
    socket.log(`INCOMING minecraft:use_item / 64 packet (${length} bytes)`)
    ...
    if (socket.playerInventory.selected_inventory == "main") {
        ...
        if (heldItem == "minecraft:diamond") {
            ...
            packetWriter.play.set_player_inventory.buffer(socket, 2, 1, "minecraft:gunpowder")
            packetWriter.play.set_player_inventory.buffer(socket, 3, 1, "minecraft:redstone")
        ...
        if (socket.playerInventory.selected_slot_create_player_auto == 36) {
            ...
            } else if (socket.createPlayerAutoSettings.state == 1 && socket.createPlayerAutoSettings.syncronization_index < 0 && socket.createPlayerAutoSettings.play_time >= 1) {
