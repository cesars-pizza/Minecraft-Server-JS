...
function read(data, length, socket, state) {
    ...
    if (socket.playerInventory.selected_inventory == "main") {
        ...
        else if (slot.value == 38) packetWriter.play.set_player_inventory.buffer(socket, 2, 1, "minecraft:gunpowder")
        else if (slot.value == 39) packetWriter.play.set_player_inventory.buffer(socket, 3, 1, "minecraft:redstone")
        ...
        else if (slot.value == 39) packetWriter.play.set_player_inventory.buffer(socket, 3, Number(socket.createPlayerAutoSettings.state > 0), "minecraft:redstone")
        else if (slot.value == 40) packetWriter.play.set_player_inventory.buffer(socket, 4, 1, "minecraft:tnt")
        else if (slot.value == 41) packetWriter.play.set_player_inventory.buffer(socket, 5, 0, null)
